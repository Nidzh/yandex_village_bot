...
async def time_16_00(test: bool = False):
    ...
    title = (
        ...
        f"✍️ После того как нажмёшь кнопку {_B('📱 Готов(а) отвечать')}, "
    ...
    kb = InlineKeyboardBuilder()
    kb.button(text="📱 Готов(а) отвечать", callback_data="answer_fairytale")
    markup = kb.as_markup() if kb else None
...
async def time_18_00(test: bool = False):
    ...
    kb = InlineKeyboardBuilder()
    kb.button(text="📱 Готов(а) отвечать", callback_data="answer_cocktails")
    markup = kb.as_markup() if kb else None
